...
describe('Trie', () => {
  ...
  it('should add words to trie', () => {
    ...
    expect(trie.head.toString()).toBe('*:c');
    expect(trie.head.getChild('c').toString()).toBe('c:a');
    // expect(trie.head.getChild('c').getChild('a').toString()).toBe('a:t,r'); // 错误的代码
  ...
  it('should suggests next characters', () => {
    ...
    // expect(trie.suggestNextCharacters('ca')).toEqual(['t', 'r', 'p']); // 错误测
    expect(trie.suggestNextCharacters('ca')).toEqual(['p', 'r', 't']); // 错误测
